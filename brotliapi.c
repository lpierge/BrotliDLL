/*
	...
	This is the modified version of the original 'brotli.c' file for the Brotli DLL interface.
	I did not write the following code, I just modified it to make it work for the DLL interface.
	Modifications are marked with the //LPI tag, minimalism is the key.

	File Placement:
	Once the official package (brotli-master.zip) is installed, the original 'brotli.c' 
	resides in:
		\brotli\c\tools
	Whereas this file ('brotliapi.c') — the modified version of 'brotli.c' — must reside in:
		\brotlidll
	along with the rest of the Visual Studio project files.

	How it works:
	The DLL source code (see 'brotlidll.c') exposes two functions:
	BrotliCompressFile() and BrotliUncompressFile()
	These functions perform file compression/decompression by invoking the modified version 
	of main(), implemented here as 'ex_main()', essentially simulating a command-line utility
	call.

	In theory, this approach should allow you to simply download the latest version of the 
	Brotli package into the 'L:\brotli' directory and recompile the DLL seamlessly.
	However, in practice, IT DOES NOT. Please refer to the extensive notes in 'brotlidll.c' 
	regarding the source files from the original package that must be duplicated, modified, 
	etc., in order to successfully compile under Windows.

	Version Info:
	The Brotli package version currently used (as of 2026) is 1.2.0, which is the latest 
	available on the official GitHub repository: https://github.com/google/brotli

	For further details, see the notes in 'brotlidll.c'.

	Luca Piergentili, 05/2026
*/
...
//LPI
*/
int ex_main(int argc,const char** argv) {
  Command command;
  Context context;
  BROTLI_BOOL is_ok = BROTLI_TRUE;
  int i;
